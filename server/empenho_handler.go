...
func (s *server) CreateEmpenhoHandler(c *gin.Context) {
	...
	location := path.Join(Prefix, "empenhos", strconv.Itoa(model.GetID()))
...
func (s *server) GetEmpenhoHandler(c *gin.Context) {
	...
	model := &schema.Empenho{StorageModel: schema.StorageModel{ID: id}}
