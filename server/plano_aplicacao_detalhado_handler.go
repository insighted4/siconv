...
func (s *server) CreatePlanoAplicacaoDetalhadoHandler(c *gin.Context) {
	...
	location := path.Join(Prefix, "plano-aplicacao-detalhados", strconv.Itoa(model.GetID()))
...
func (s *server) GetPlanoAplicacaoDetalhadoHandler(c *gin.Context) {
	...
	model := &schema.PlanoAplicacaoDetalhado{StorageModel: schema.StorageModel{ID: id}}
